...
def lock_block_reason(distance_value, target_in_view, bbox_in_lock_box):
    # Keep mapping strictly aligned with spec for debug.
    if distance_value > 30.0:
        return "distance_too_far"
    if distance_value <= 30.0 and not bbox_in_lock_box:
        return "bbox_not_centered"
    if distance_value <= 30.0:
        return "close_range_destroy"
...
def perform_drop_effect(client, chaser_name="Chaser", target_name="Target", duration_seconds=5.0):
    print(
        "[DEMO FINAL] "
    ...
    while time.monotonic() < end_time:
        try:
            ...
            pass
        try:
            # AirSim NED: +Z velocity is downward.
...
def main():
    ...
    try:
        ...
        for step_index in range(1, max_steps + 1):
            ...
            bbox_center = camera_state.get("bbox_center")

            # Lock/drop only. Does not affect planner/safety/chaser speed.
            # Priority (given by spec):
            # 1) distance <= 3.0  => close_catch (no bbox/visibility requirement)
            # 2) distance <= 30.0 => close_range_destroy (no bbox/visibility requirement)
            # 3) distance <= 30.0 => camera_lock_30m only if target_in_view=True and bbox_center not None and bbox_in_lock_box=True

            distance_le_3 = distance_value <= 3.0
            distance_le_30 = distance_value <= 30.0
            bbox_center_not_none = bbox_center is not None


            camera_lock = False
            done_reason = None
            lock_block_reason_value = lock_block_reason(
                distance_value,
                target_in_view,
                bbox_in_lock_box,
            )
            ...
            if distance_le_3:
                camera_lock = True
                done_reason = "close_catch"
            # close_range_destroy
            elif distance_le_30:
                camera_lock = True
                done_reason = "close_range_destroy"

            # camera_lock_30m
            elif distance_le_30 and target_in_view and bbox_center_not_none and bbox_in_lock_box:
                camera_lock = True
                done_reason = "camera_lock_30m"
            ...
            if camera_lock:
                ...
                caught = True
                final_reason = done_reason or "camera_lock"
                ...
                info["lock_ready"] = True
            elif bool(info.get("caught", False)):
                caught = True
                final_reason = info.get("done_reason", "caught")
            
            # DEBUG extra signals for lock logic (no control flow impact)
            camera_state["target_in_view"] = target_in_view
            camera_state["bbox_in_lock_box"] = bbox_in_lock_box
            camera_state["bbox_center_not_none"] = bbox_center_not_none
            camera_state["lock_block_reason"] = lock_block_reason_value
            camera_state["lock_ready"] = bool(camera_lock)
